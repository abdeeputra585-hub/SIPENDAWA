...
const navigationMenus = {
    ...
    guru: [
        ...
        { id: 'admin-izin', label: 'Kehadiran & Izin', icon: 'fa-user-check' },
        { id: 'pesan', label: 'Pesan / Obrolan', icon: 'fa-comments', badge: 0 },
        { id: 'admin-pengumuman', label: 'Pengumuman', icon: 'fa-bullhorn' },
        { id: 'notifikasi', label: 'Notifikasi', icon: 'fa-bell', badge: 0 },
    ...
    parent: [
        ...
        { id: 'pesan', label: 'Pesan Guru', icon: 'fa-comments', badge: 0 },
...
function loginAs(role, userData) {
    ...
    if (typeof initGlobalpesanPolling === 'function') initGlobalpesanPolling();
}
...
function switchTab(tabId) {
    ...
    if (tabId === 'pesan') { 
        if (typeof showpesanPage === 'function') showpesanPage(); 
    } else {
        if (typeof pesanPollingInterval !== 'undefined' && pesanPollingInterval) {
...
async function loadLaporanData(kelas = '', dari = '', sampai = '') {
    try {
        ...
        if (data.data.length > 0) {
            tbody.innerHTML = '';
            laporanDataCache = data.data; // Simpan ke cache agar export PDF/CSV berfungsi
            data.data.forEach(l => {
                ...
                tbody.innerHTML += `<tr class="hover:bg-slate-50/50">
                    <td class="px-4 py-3"><div class="font-bold text-slate-900">${l.wali_nama}</div><div class="text-3xs text-slate-400 font-medium">${l.telepon || '-'}</div></td>
                    <td class="px-4 py-3"><div class="text-slate-900">${l.siswa_nama}</div><div class="text-3xs text-slate-400 font-medium">Kelas: ${l.kelas || '-'}</div></td>
                    <td class="px-4 py-3 text-center"><span class="text-3xs font-bold ${tipeStyle} px-1.5 py-0.5 rounded">${l.hubungan}</span></td>
                    <td class="px-4 py-3"><span class="flex items-center gap-1 text-3xs ${l.status === 'Terverifikasi' ? 'text-emerald-600' : 'text-amber-600'}"><span class="w-1.5 h-1.5 rounded-full ${l.status === 'Terverifikasi' ? 'bg-emerald-500' : 'bg-amber-500'}"></span> ${l.status}</span></td>
                    <td class="px-4 py-3 text-slate-600 font-medium">${tgl}</td>
                </tr>`;
            });
        } else {
            laporanDataCache = []; // Kosongkan cache
...
function filterLaporan() {
    ...
    const tbody = document.querySelector('#page-admin-laporan table tbody');
    if (tbody) {
        tbody.innerHTML = `<tr><td colspan="5" class="px-4 py-8 text-center text-slate-400 italic text-xs"><i class="fa-solid fa-spinner animate-spin mr-2"></i>Memuat data...</td></tr>`;
    }
...
async function loadParentNilaiData() {
    ...
    try {
        ...
        // Ambil filter secara aman (fallback jika elemen tidak ada di HTML)
        const tahunEl = document.getElementById('parent-filter-tahun');
        const semesterEl = document.getElementById('filter-parent-nilai-semester') || document.getElementById('parent-filter-semester');
        const bulanEl = document.getElementById('parent-filter-bulan');
        ...
        if (dataNilai.success && dataNilai.data.length > 0) {
            const sum = dataNilai.data.reduce((acc, curr) => acc + parseFloat(curr.nilai || curr.nilai_akhir), 0);
            avgNilai = sum / dataNilai.data.length;
            hasLowGrades = dataNilai.data.some(n => parseFloat(n.nilai || n.nilai_akhir) < 70);
        }
        ...
        // Render Cards
        const avgNilaiEl = document.getElementById('parent-avg-nilai');
        if (avgNilaiEl) avgNilaiEl.textContent = avgNilai.toFixed(1);
        
        const avgHadirEl = document.getElementById('parent-avg-hadir');
        if (avgHadirEl) avgHadirEl.textContent = percentHadir + '%';
        
        const totalAlpaEl = document.getElementById('parent-total-alpa');
        if (totalAlpaEl) totalAlpaEl.textContent = totalAlpa + ' Hari';
        ...
        if (dataNilai.success) {
            ...
            dataNilai.data.forEach(item => {
                const nAkhir = parseFloat(item.nilai || item.nilai_akhir).toFixed(1);
...
async function loadParentCatatan() {
    const listDOM = document.getElementById('grid-parent-catatan') || document.getElementById('list-parent-catatan');
    if (!listDOM) return;
    listDOM.innerHTML = '<div class="col-span-full text-center text-slate-500 text-xs py-4">Memuat catatan harian...</div>';
...
async function loadDataKelas() {
    ...
    if (!container) return;
    container.innerHTML = '<div class="bg-white border border-slate-200 rounded-xl shadow-3xs p-8 text-center text-slate-400"><i class="fa-solid fa-spinner animate-spin mr-2"></i> Memuat data...</div>';
    ...
    try {
        ...
        if (data.success) {
            window.kelasDataList = data.data; // Simpan ke global variable
            if (data.data.length === 0) {
                container.innerHTML = '<div class="bg-white border border-slate-200 rounded-xl shadow-3xs p-8 text-center text-slate-400">Belum ada data kelas</div>';
                return;
            }
            ...
            const groupedData = {};
            data.data.forEach(k => {
                if (!groupedData[k.tingkat]) {
                    groupedData[k.tingkat] = [];
                }
                groupedData[k.tingkat].push(k);
            });

            const tingkats = Object.keys(groupedData).sort((a, b) => parseInt(a) - parseInt(b));
            let htmlStr = '';

            tingkats.forEach(tingkat => {
                const kelasList = groupedData[tingkat];
                htmlStr += `
                <div class="bg-white border border-slate-200 rounded-xl shadow-3xs overflow-hidden">
                    <div class="bg-slate-50 border-b border-slate-200 px-6 py-4 flex justify-between items-center">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="font-bold text-slate-800 text-sm">Tingkat ${tingkat}</h3>
                        </div>
                        <span class="text-xs font-semibold bg-white border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg shadow-sm">${kelasList.length} Kelas</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-xs">
                            <thead class="bg-slate-50/30 border-b border-slate-100">
                                <tr class="text-3xs font-bold text-slate-400 uppercase tracking-wider">
                                    <th class="px-6 py-3 text-left w-1/3">Nama Kelas</th>
                                    <th class="px-4 py-3 text-left w-1/3">Wali Kelas</th>
                                    <th class="px-4 py-3 text-center">Siswa</th>
                                    <th class="admin-only px-4 py-3 text-center">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100">
                                ${kelasList.map(k => `
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        <td class="px-6 py-3 font-bold text-blue-700">${k.nama_kelas}</td>
                                        <td class="px-4 py-3 text-slate-700 font-medium">${k.wali_kelas_nama ? k.wali_kelas_nama : '<span class="text-slate-400 italic">Belum diatur</span>'}</td>
                                        <td class="px-4 py-3 text-center ${k.jumlah_siswa > 0 ? 'text-slate-900 font-bold' : 'text-slate-400'}">
                                            ${k.jumlah_siswa > 0 ? `<button onclick="lihatSiswaKelas('${k.nama_kelas}')" class="text-blue-600 hover:text-blue-800 hover:underline px-2 py-1 rounded transition-colors" title="Lihat daftar siswa">${k.jumlah_siswa} Siswa</button>` : '-'}
                                        </td>
                                        <td class="px-4 py-3 text-center admin-only">
                                            <div class="flex items-center justify-center gap-2">
                                                <button onclick="editKelas(${k.id})" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center transition-colors"><i class="fa-solid fa-pen"></i></button>
        ...
        container.innerHTML = '<div class="bg-white border border-slate-200 rounded-xl shadow-3xs p-8 text-center text-red-500">Gagal memuat data</div>';
    }
}

async function fetchGuruSiswaOptions(selectedGuruId = null, selectedSiswaIds = []) {
    const waliSelect = document.getElementById('kelas-wali');
    const siswaList = document.getElementById('kelas-siswa-list');
    
    // Set loading
    waliSelect.innerHTML = '<option value="">Memuat...</option>';
    siswaList.innerHTML = '<div class="text-xs text-slate-400 italic text-center py-4"><i class="fa-solid fa-spinner animate-spin"></i> Memuat daftar siswa...</div>';
    
    try {
        const [resGuru, resSiswa] = await Promise.all([
            fetch(`${API_BASE}/admin/guru/list_all.php`, { headers: authHeaders() }),
            fetch(`${API_BASE}/admin/siswa.php`, { headers: authHeaders() })
        ]);
        ...
        if (dataGuru.success) {
            let guruHtml = '<option value="">-- Tidak Ada / Belum Diatur --</option>';
            dataGuru.data.forEach(g => {
                const selected = (selectedGuruId == g.guru_profile_id) ? 'selected' : '';
                guruHtml += `<option value="${g.guru_profile_id}" ${selected}>${g.nama} (NIP: ${g.nip})</option>`;
            });
            waliSelect.innerHTML = guruHtml;
        }
        
        if (dataSiswa.success) {
            if (dataSiswa.data.length === 0) {
                siswaList.innerHTML = '<div class="text-xs text-slate-400 italic text-center py-4">Belum ada siswa aktif</div>';
            } else {
                let siswaHtml = '';
                dataSiswa.data.forEach(s => {
                    const checked = selectedSiswaIds.includes(parseInt(s.id)) ? 'checked' : '';
                    siswaHtml += `
                        <label class="flex items-center gap-3 p-2 hover:bg-slate-100 rounded-lg cursor-pointer transition-colors border border-transparent hover:border-slate-200">
                            <input type="checkbox" name="kelas_siswa_ids[]" value="${s.id}" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" ${checked}>
                            <div class="flex flex-col">
                                <span class="text-xs font-bold text-slate-800">${s.nama}</span>
                                <span class="text-3xs text-slate-500">NISN: ${s.nisn} | L/P: ${s.jenis_kelamin === 'Laki-laki' ? 'L' : 'P'}</span>
                            </div>
                        </label>
                    `;
                });
...
function openModalKelas() {
    ...
    document.getElementById('kelas-wali').innerHTML = '<option value="">Memuat...</option>';
    document.getElementById('kelas-siswa-list').innerHTML = '<div class="text-xs text-slate-400 italic text-center py-4"><i class="fa-solid fa-spinner animate-spin"></i> Memuat daftar siswa...</div>';
    document.getElementById('modal-kelas').classList.remove('hidden');
    fetchGuruSiswaOptions();
}
...
async function simpanKelas(e) {
    ...
    const guru_profile_id = document.getElementById('kelas-wali').value;
    
    // Collect selected siswa IDs
    const checkboxes = document.querySelectorAll('input[name="kelas_siswa_ids[]"]:checked');
    ...
    const payload = { 
        ...
        nama_kelas, 
        tingkat,
        guru_profile_id: guru_profile_id ? parseInt(guru_profile_id) : 0,
        siswa_ids 
    };
...
function editKelas(id) {
    const k = window.kelasDataList.find(x => x.id == id);
    if (!k) return;
    ...
    fetchGuruSiswaOptions(k.guru_profile_id, k.siswa_ids);
}
...
// Reset dark mode ke light secara otomatis
(function(){
    localStorage.setItem('SIPENDAWA-dark', 'false');
    document.documentElement.classList.remove('dark');
})();

async function lihatSiswaKelas(namaKelas) {
    document.getElementById('modal-lihat-siswa-title').innerText = "Daftar Siswa Kelas " + namaKelas;
    document.getElementById('modal-lihat-siswa').classList.remove('hidden');
    const container = document.getElementById('lihat-siswa-list');
    container.innerHTML = '<div class="text-center text-slate-400 italic text-sm py-8"><i class="fa-solid fa-spinner animate-spin mr-2"></i> Memuat daftar siswa...</div>';

    try {
        const res = await fetch(`${API_BASE}/admin/siswa.php`, { headers: authHeaders() });
        const data = await res.json();
        if (data.success) {
            const siswaKelasIni = data.data.filter(s => s.kelas === namaKelas);
            if (siswaKelasIni.length === 0) {
                container.innerHTML = '<div class="text-center text-slate-400 italic text-sm py-8">Tidak ada siswa di kelas ini</div>';
            } else {
                let html = '';
                siswaKelasIni.forEach(s => {
                    html += `<div class="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl shadow-3xs">
                            <div class="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold shrink-0">
                                ${s.nama.charAt(0)}
                            </div>
                            <div class="flex-1">
                                <h4 class="text-sm font-bold text-slate-800">${s.nama}</h4>
                                <p class="text-3xs text-slate-500">NISN: ${s.nisn} &bull; ${s.jenis_kelamin}</p>
                            </div>
                        </div>`;
                });
            ...
            container.innerHTML = '<div class="text-center text-red-500 text-sm py-8">Gagal memuat data siswa</div>';
        }
    } catch (err) {
